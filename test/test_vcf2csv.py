...
import numpy as np
import pandas as pd
from somatic_pipeline.vcf2csv import Vcf2Csv, GetInfoIDToDescription, VcfLineToRow, UnrollVEPAnnotation, GetAllColumns
...
class TestVcf2Csv(TestCase):
    ...
    def test_main(self):
        actual = Vcf2Csv(self.settings).main(
            vcf=f'{self.indir}/variants.vcf',
            dstdir=self.workdir
        )
        expected = f'{self.indir}/variants.csv'
...
class TestUnrollVEPAnnotation(TestCase):
    ...
    def test_mock(self):
        d = {
            "Consequence annotations from Ensembl VEP. Format: Allele|Consequence|IMPACT|SYMBOL|BAM_EDIT":
                'A|consequence(1)|impact(1)|symbol(1)|bam_edit(1),A|consequence(2)|impact(2)|symbol(2)|bam_edit(2)'
        }
        actual = UnrollVEPAnnotation(self.settings).main(row=d)
        expected = {
            'Allele': 'A',
    ...
    def test_real(self):
        ...
        expected = {
            ...
            'IMPACT': 'MODIFIER',
            'SYMBOL': 'WASHC1',
            'Gene': 'ENSG00000181404',
            'Feature_type': 'Transcript',
            'Feature': 'ENST00000442898',
